...
class OrderServiceTest {
        ...
        void shouldThrowExceptionWhenProductIsNotAvailable() {
                CreateOrderRequest request = new CreateOrderRequest(
                                List.of(new CreateOrderItemRequest(999L, 1)));

                when(productClient.getProductById(999L))
                                .thenThrow(new ProductNotAvailableException(999L));

                ProductNotAvailableException exception = assertThrows(
                                ProductNotAvailableException.class,
                                () -> orderService.createOrder(request));

                assertEquals("Product with id 999 is not available", exception.getMessage());
        ...
        void shouldThrowExceptionWhenInventoryReservationFails() {
                CreateOrderRequest request = new CreateOrderRequest(
                                List.of(new CreateOrderItemRequest(1L, 100)));

                when(productClient.getProductById(1L)).thenReturn(
                                new ProductResponse(1L, "Lenovo ThinkPad E16", "Laptop", new BigDecimal("899.99")));

                when(inventoryClient.reserveStock(any()))
                                .thenThrow(new InventoryReservationException("Inventory reservation failed"));
