...
Write-Host "Running Stress test - gRPC-inventory..." -ForegroundColor Cyan
Write-Host "Target: $env:BASE_URL" -ForegroundColor Yellow
Write-Host "Dashboard: http://localhost:5666" -ForegroundColor Green
Write-Host "JSON summary: results\grpc-inventory\03-stress-summary.json" -ForegroundColor Green
Write-Host "Screenshots folder: screenshots\grpc-inventory" -ForegroundColor Green
Write-Host ""

k6 run `
  --summary-export="results\grpc-inventory\03-stress-summary.json" `
  "k6-tests\rest\03-stress-test.js"
