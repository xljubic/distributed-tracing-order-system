...
Write-Host "Running Baseline load test - gRPC-inventory..." -ForegroundColor Cyan
Write-Host "Target: $env:BASE_URL" -ForegroundColor Yellow
Write-Host "Dashboard: http://localhost:5666" -ForegroundColor Green
Write-Host "JSON summary: results\grpc-inventory\02-baseline-summary.json" -ForegroundColor Green
Write-Host "Screenshots folder: screenshots\grpc-inventory" -ForegroundColor Green
Write-Host ""

k6 run `
  --summary-export="results\grpc-inventory\02-baseline-summary.json" `
